import { dataAboutPage, dataExp } from "@/data";

const LineaTiempoExp = () => {
    return (   <div className="flex flex-col justify-center divide-y divide-slate-200">
        <div className="z-20 grid items-center text-justify h-full  p-6 py-20 md:py-0 ">
            <div className="-my-6">
            <h1 className="mb-5 text-2xl leading-tight text-start  md:text-4xl md:mb-10 mt-14">
                Formacion{' '}
                ...
                {dataExp.map((data) => (
                    <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
                        ...
                        <div className="text-slate-400 text-2xl text-start">{data.description}</div>
                    </div>
                ))}
            </div>
        </div>
